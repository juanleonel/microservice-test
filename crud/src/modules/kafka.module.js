...
const consumerModule = async () => {
  ...
  await consumer.run({ eachMessage: async ({ topic, partition, message }) => {
      ...
      try {
        ...
        if (!blockedAccount) {
          const updatedAccount = await Account.findOneAndUpdate({ _id: accountId }, { $inc: { count: 1 } }, { new: true });

          if (updatedAccount.count === 3) {
            await Account.findOneAndUpdate({ _id: accountId }, { status: 'blocked' }, { new: true });
          }
        } else {
